    ...
    if (!(company.deliveryMethod.includes(userInputs.deliveryMethod) && company.source.includes(userInputs.source)) || userInputs.amount > company.maxLimit) {
        ...
        */
        if (userInputs.source == 'Agent' && userInputs.deliveryMethod == 'Agent') {
            if (userInputs.amount > 4501) { // Max Limit
                return
            } else if (userInputs.amount > 0 && userInputs.amount < 51) {
                _this.fee = 5;
            } else if (userInputs.amount > 50 && userInputs.amount < 1001) {
                _this.fee = 9.99;
            } else if (userInputs.amount > 1000 && userInputs.amount < 4502) {
                _this.fee = (userInputs.amount - 1000) * 0.02 + 21.00;
            }

        } else if (userInputs.source == 'Agent' && userInputs.deliveryMethod == 'Bank Account') {
            if (userInputs.amount > 0 && userInputs.amount < 51) {
                _this.fee = 5;
            } else if (userInputs.amount > 50 && userInputs.amount < 1001) {
                _this.fee = 9.99;
            } else if (userInputs.amount > 1000 && userInputs.amount < 4502) {
            ...
            if (userInputs.amount > 2001) { // Max Limit
                return
            } else {
                _this.fee = 0;
            }
            ...
            if (userInputs.amount > 0 && userInputs.amount < 51) {
                _this.fee = 4.99;
            } else if (userInputs.amount > 50) {
                _this.fee = 4;
            }
        } else if ((userInputs.source == 'Debit Card' || userInputs.source == 'Credit Card' ) && userInputs.deliveryMethod == 'Agent') {
            if (userInputs.amount > 0 && userInputs.amount < 301) {
                _this.fee = 4.99;
            } else if (userInputs.amount > 300 && userInputs.amount < 601) {
                _this.fee = 6.99;
            } else if (userInputs.amount > 601 && userInputs.amount < 4500) {
                _this.fee = 0.99;
            }
        }
